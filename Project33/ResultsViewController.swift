    ...
    override func viewDidLoad() {
        ...
        CKContainer.default().publicCloudDatabase.perform(query, inZoneWith: nil) {
            ...
            if let error = error {
                ...
                let ac = UIAlertController(title: "Error", message: "There was a problem querying entries: \(error.localizedDescription)", preferredStyle: .alert)
        ...
        CKContainer.default().publicCloudDatabase.fetch(withRecordID: whistle.recordID) {
            ...
            if let error = error {
                DispatchQueue.main.async {
                    let ac = UIAlertController(title: "Error", message: "Unable to download recording. \(error.localizedDescription)", preferredStyle: .alert)
                    ac.addAction(UIAlertAction(title: "OK", style: .default))
                    self.present(ac, animated: true)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            if indexPath.row == suggestions.count {
                cell.textLabel?.text = "Add suggestions"
        ...
        CKContainer.default().publicCloudDatabase.save(whistleRecord) {
            ...
            DispatchQueue.main.async {
                ...
                if error == nil {
                    ...
                    let ac = UIAlertController(title: "Error", message: "There was a problem submitting your suggestions: \(error!.localizedDescription)", preferredStyle: .alert)
                    ac.addAction(UIAlertAction(title: "OK", style: .default))
                    self.present(ac, animated: true)
                }
